...
The maintainer scripts support Noble, Plucky, Resolute, Trixie, and Forky. On an
unsupported codename the package installs normally but does not create a source.
After an OS upgrade, `sudo dpkg-reconfigure winegui` updates a source that still
matches WineGUI's last generated content. An administrator-edited source is
preserved; update its `Suites:` field manually when switching to a different
suite. Deleting the source is a persistent opt-out.
