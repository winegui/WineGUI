...
{
  const std::vector<std::string> config_dirs{Glib::get_user_config_dir(), "winegui"};
  std::string config_location = Glib::build_path(G_DIR_SEPARATOR_S, config_dirs);
  std::string config_file_path = Glib::build_filename(config_location, "config.ini");
  const std::vector<std::string> prefix_dirs{Glib::get_user_data_dir(), "winegui", "prefixes"};
  std::string default_prefix_folder = Glib::build_path(G_DIR_SEPARATOR_S, prefix_dirs);

  // Check if config folder directory exists
  if (!Glib::file_test(config_location, Glib::FileTest::IS_DIR))
  {
...
std::string GeneralConfigFile::config_and_folder_migration(const std::string& config_file_path_new, const std::string& default_prefix_folder_new)
{
  ...
  const std::vector<std::string> config_dirs_old{Glib::get_home_dir(), ".winegui"};
  ...
  const std::vector<std::string> prefix_dirs_old{Glib::get_home_dir(), ".winegui", "prefixes"};
  ...
  if (Glib::file_test(config_location_old, Glib::FileTest::IS_DIR))
  {
    // Check on old config.ini file
    if (Glib::file_test(config_file_path_old, Glib::FileTest::IS_REGULAR))
    {
      ...
      if (has_error)
      {
        ...
        if (Glib::file_test(config_file_path_new, Glib::FileTest::IS_REGULAR))
        {
    ...
    if (Glib::file_test(winetricks_file_path_old, Glib::FileTest::IS_REGULAR))
    {
    ...
    if (Glib::file_test(winetricks_file_path_bak_old, Glib::FileTest::IS_REGULAR))
    {
    ...
    if (Glib::file_test(default_prefix_folder_old, Glib::FileTest::IS_DIR))
    {
      try
      {
